...
実行日: 2026-09-22 ／ 固定時刻 2026-09-16 06:50 (Asia/Manila) ／ Chromium 390×844
...
合計 89 項目 ／ NG 0 件 ／ ページエラー mock 0 件・db 1 件
...
| db スキーマ: log_weight{value,unit,skipped,skipReason,loggedAt} / warmup{completed,minutes,loggedAt} / meals{status: plan|substitute|skip|photo, photoId, reason} / log_daily{dayNo,dayName,notes,skippedItems[]} | ✓ | ✓ | db のみ / statuses=plan,plan,substitute / 9/16 skippedItems= |
| 英略語（W/MAIN/TOP/BO/DROP/PRE/FINAL）がUIに出ない | ✓ | ✓ |  |
| 数だけの表示（7種、1種）がない | ✓ | ✓ |  |
| 同日に2回開いても二重保存されない | ✓ | ✓ | db のみ / docs: {"log_workout":4,"plan_days":7,"plan_exercises":54,"plan_warmup":6,"plan_meals":5,"plan_supplements":8,"plan_routine":3,"foods":36,"settings":1,"log_weight":2,"log_daily":5,"log_routine":2,"log_meals":1,"log_supplements":2,"log_media":3,"log_cardio":2} |
...
| 文字のAI推定（AI計算・文字で推定）がコードに残っていない。写真の推定だけが sample を使う | ✓ | ✓ | 文字のAI推定は完全に削除 / 文字のAI推定は完全に削除 |
...
| リンゴ酢✓ → 「1食目まで あと 15:00」のカウンターが「いま」カードとタイムラインの両方に出る。15分たっても消さず「15分経過 ・ 1食目OK（経過 ◯分）」で残り、閉じて開き直しても残り時間が正しい | ✓ | ✓ | 15分カウンター（いま＋項目）・15分後も残る・再読み込みで正しい・取り消しと時刻修正で再計算 / 15分カウンター（いま＋項目）・15分後も残る・再読み込みで正しい・取り消しと時刻修正で再計算 |
...
| 1日合計が画面下部に固定で出る。タップで展開して脂肪の塊と週まとめへの導線（1食目を手計算で検算） | ✓ | ✓ | 1食目 = 372.5kcal / P40.4 F20.6 C1.7 と一致。固定バーは今日だけ / 1食目 = 372.5kcal / P40.4 F20.6 C1.7 と一致。固定バーは今日だけ |
| 今日のプランは「食べた（✓）／食べてない（○）」の2つだけ。予定時刻を過ぎてもオレンジや警告を出さず、プラン外を記録してもプランは一切変わらない（打ち消し線・変更あり・プラン比なし） | ✓ | ✓ | プランは食べた／食べてないの2つだけ。プラン外の記録はプランに影響しない / プランは食べた／食べてないの2つだけ。プラン外の記録はプランに影響しない |
| 今日以外の日付では「いま」カードも記録ボタンも出さず、✓ も押せない。過去の記録は長押し →「修正する」で開く | ✓ | ✓ | 過去・未来では「いま」カード・記録ボタンを出さず ✓ も無効。長押しの「修正する」だけで開き、時刻は元の記録のまま / 過去・未来では「いま」カード・記録ボタンを出さず ✓ も無効。長押しの「修正する」だけで開き、時刻は元の記録のまま |
| 実績時刻: ルーティンは記録済みが太字＋「予定」、60分以上ズレたら黄色。食べたものは時刻をタップで直せて並び順も変わる | ✓ | ✓ |  |
...
| 15分カウンターは次の行動を記録したときだけ消え「◯分後に…」が残る。15分より早く進めたら「（15分より早い）」を添える。15分に達したらその場で知らせる | ✓ | ✓ | リンゴ酢 18分後に1食目 ／ 経口サプリ 9分後にサイリウム（15分より早い） / リンゴ酢 18分後に1食目 ／ 経口サプリ 9分後にサイリウム（15分より早い） |

## 食事シート

| 項目 | mock | db | 備考 |
|---|:-:|:-:|---|
| プランのカードをタップ →「全部食べた」で1タップ記録。✓ が付いて合計に反映し、「食べたもの」に1件増える | ✓ | ✓ |  |
| 4食目の代替案（白米150・鶏100・全卵1）を選べる | ✓ | ✓ |  |
| 米の基準（生/炊飯後）表示が settings に連動（記録済みは食べたときの基準のまま、未記録の予定は設定に追従） | ✓ | ✓ |  |
| プランの状態は「食べた（✓）／食べてない（○）」の2つだけ。スキップ・変更・プラン比のバッジは出さない | ✓ | ✓ |  |
| プランの記録は ✓ の付け外しで何度でも往復でき、毎回 合計が元に戻る | ✓ | ✓ | base=1571 kcal に毎回戻る / base=1571 kcal に毎回戻る |
| 品目選択シート: 品目ごとにチェックと分量を決めて記録する／長押しで ×0.5・×1.5・×2／全卵は「個」ホエイは「スクープ」／食品マスタから追加・新規登録／抜いた品目は打ち消し線でプランとの差も出す | ✓ | ✓ | 品目ごとのチェックと分量・即時再計算・長押しの倍率・個/スクープ入力・食品マスタからの追加と新規登録・打ち消し線とプラン差、すべて確認 / 品目ごとのチェックと分量・即時再計算・長押しの倍率・個/スクープ入力・食品マスタからの追加と新規登録・打ち消し線とプラン差、すべて確認 |
| カロリーを直接入力: どの食事かは聞かず「食べたもの」に1件増えるだけでプランは変わらない／カロリー必須・品目名は任意／タンパク質などから計算／合計と脂肪の塊に反映→取り消しで消える | ✓ | ✓ | どの食事かを聞かず、プランに触れずに「食べたもの」へ1件追加・カロリー必須・PFCから計算 / どの食事かを聞かず、プランに触れずに「食べたもの」へ1件追加・カロリー必須・PFCから計算 |
| 記録画面: よく食べるものをチェックで複数選んで1件の記録にできる。個数は 0.5 単位、合計はその場で変わる。すべての食品に分量と単位が付き、重複が無い | ✓ | ✓ | チェックで複数選択 → 1件の記録。0.5 単位・新規登録・カロリーだけ入力を混ぜられる / チェックで複数選択 → 1件の記録。0.5 単位・新規登録・カロリーだけ入力を混ぜられる |
| 記録ボタンは1つにまとまり、開いた瞬間に検索へフォーカスが当たる。「食べたもの」から削除すると、プランの記録ならプランも ○ に戻る | ✓ | ✓ | 記録ボタン1つ・フォーカス・削除でプランも ○ に戻る / 記録ボタン1つ・フォーカス・削除でプランも ○ に戻る |
| 食品マスタの重複をまとめる移行: 過去の記録は消さず、まとめた先の食品に付け替わる。よく食べるものは使った回数の多い順に並ぶ | ✓ | ✓ | db のみ（移行は db 上の既存データに対して走る） / ピスタチオ 1・シリアル → まとめ先に付け替え。よく使う順に並ぶ |
| プランの「まとめて記録」で複数の食事を選び、同じ時刻でそれぞれ別の記録として入れられる。記録の時刻はタップで直せる | ✓ | ✓ | まとめて記録（同じ時刻・別々の記録）・記録画面の時刻修正 / まとめて記録（同じ時刻・別々の記録）・記録画面の時刻修正 |
...
| 休みの日は「その日を過ごした記録」があれば消化: 食事だけ・体重だけでも翌日に持ち越さない／何も無ければ持ち越す／「有酸素はやらない」で消化／筋トレの日は記録があっても未完了なら持ち越す。休みの日の目標カロリーも平日と同じ | ✓ | ✓ | 休みの日は 有酸素／食事／体重／サプリ／「有酸素はやらない」のどれかで消化。筋トレの日は今までどおり完了が必要。休みの日の目標も 2,632kcal / P210 F69 C295 / 休みの日は 有酸素／食事／体重／サプリ／「有酸素はやらない」のどれかで消化。筋トレの日は今までどおり完了が必要。休みの日の目標も 2,632kcal / P210 F69 C295 |
...
| 不具合修正: 完了・スキップは記録だけから判定する。全セット記録済みなら「セット 3/3 完了」＋完了カード＋「次の種目へ」／記録済み行をタップすると「記録を上書きする」／再訪時のカーソルは最初の未記録セット／「筋トレ完了」を押していなくても記録が揃っていれば Day は消化される | ✓ | ✓ | 記録済みセットから完了を判定（フラグを持たない） / 記録済みセットから完了を判定（フラグを持たない） |
| コーチのフォーム指摘: 種目名のすぐ下・セット入力の上に折りたたまずに出す（ショルダープレス・トライセップ プッシュダウン・カーフ3種）。「コーチより 9/21」・動画は別タブ・新しい指摘は最初の1回だけ強調。カーフの提案は前回20回で+10kg・18回で+5kg | ✓ | ✓ | カーフ 20回→+10kg / 18回→+5kg / 15回→+2.5kg / 14回→据え置き / カーフ 20回→+10kg / 18回→+5kg / 15回→+2.5kg / 14回→据え置き |
| 不具合修正: 筋トレの日（Day 6 Pull）が「休み」にならない。手で休みに変えた日はその旨を出して1タップで戻せる。有酸素の説明から「確認中」が消えている | ✓ | ✓ | Day 6 は筋トレの日。手で休みにした日は戻せる / Day 6 は筋トレの日。手で休みにした日は戻せる |
...
| CSV エクスポート。downloads が null ならボタン非表示 | ✓ | ✓ | downloads null → 非表示 / training-log-2026-09-16.csv (15809 bytes) |
...
| 項目 | mock | db | 備考 |
|---|:-:|:-:|---|
| 写真→推定→修正→記録→合計に反映→取り消し→合計から消える | ✓ | ✓ | sample null → 写真の選択肢を出さない（文字入力のみ） / 1280×853 JPEG 10KB を送信 / 白ご飯 210g=328kcal※ を記録 |
| 「＋ 食べたものを記録」→ 写真から推定 → 「食べたもの」に記録した時刻で並び、合計に反映。プランは一切変わらない。取り消しで消える | ✓ | ✓ | sample null → 写真の選択肢なし / 写真 → 食べたものに1件（時刻順・番号なし・※推定）。プランには触れない |
| 推定 JSON の parse 失敗時にアプリが落ちず、手入力に切替できる | ✓ | ✓ | 写真ボタン非表示のため対象外 |
| 画像を送れない画面（limits に images が無い）では「写真から推定」を出さない。設定に「写真推定：使える／使えない」が出る | ✓ | ✓ | db のみ（sample を注入して判定する） / 写真推定：使えない（この画面では画像を送れません） |
| sample が画像を受け付けない環境の逃げ道: Claude のチャットで写真を見てもらい、返ってきた JSON を貼り付けると、写真から推定したときと同じ結果画面になる（```json の囲みや前後の文章が付いていても読める） | ✓ | ✓ | db のみ / Claude のチャットの返事を貼るだけで、写真と同じ結果画面・同じ記録になる |
| 写真の送信に失敗したら、実際に返った code を画面に出す（images_unavailable なら写真の入口も消す）。自動の再試行はしない | ✓ | ✓ | db のみ / code をそのまま表示 → 写真の入口を隠す（次回から静かに案内する） |
| 解析中は「写真を解析しています…（最大1分ほどかかります）」と「止める」を出し、止めたら元に戻る。補足を足して推定し直せる（自前のタイムアウトは入れない） | ✓ | ✓ | db のみ / 解析中の案内と中止、結果からの補足つき推定し直し |
| レート制限など他の失敗も code を画面に出し「もう一度」「チャットで見てもらう」「手で選んで記録」を選べる／JPEG に変換できない写真は image_rejected として案内する（accept に capture は付けない） | ✓ | ✓ | db のみ / rate_limited / image_rejected とも code を画面に出す |
