...
- **Claude のチャットに貼る方式**（`pasteFormHtml` / `chatPrompt` / `parseEstimateText`）: この環境の `sample` は画像をいっさい受け付けない（アプリ内表示でも Safari でも `images_unavailable`）ため、「Claude のチャットで写真を見てもらう」→ 指示文をコピー → チャットに写真と一緒に貼る → 返事をまるごと貼り戻す、という経路を用意した。コードブロックの囲みや前後の文章が付いていても JSON を取り出し、`validateEstimate` に通すので、**結果画面・分量の修正・記録の保存は写真から推定したときと完全に同じ**
